//Stores peak of the day in a log file
...
//	strcpy(hour,argv[1]);
	...
	for (int i = 0; i < if_count; i++)
	{
		fprintf(fp,"%s,%s,%.2f,%.2f\n",date,if_list[i].name,if_list[i].rx,if_list[i].tx);
	}
	printf("Save peaks of %i interfaces\n",if_count);
	fclose(fp);

/*	for (int i = 0; i < if_count; i++)
	{
*/		/* Creating query for saving peaks in the database */
/*		char saveQuery[] = "INSERT INTO hourPeak(Time,Name,rx,tx)VALUES(\'%s\',\'%s\',%.2f,%.2f);";
